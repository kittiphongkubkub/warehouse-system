...
import { Plus, ArrowDownToLine, Eye } from 'lucide-react'
...
export default function StockInPage() {
  ...
  useEffect(() => {
    fetch('/api/stock-in').then(r => r.json()).then(d => { setData(d); setLoading(false) })
  }, [])
  ...
  return (
    <div className="fade-in">
      <div className="page-header">
        <div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 700, margin: 0 }}>รับสินค้าเข้า</h1>
          <p style={{ color: '#64748b', margin: '4px 0 0', fontSize: '0.875rem' }}>ทั้งหมด {data.total} เอกสาร</p>
        </div>
        <Link href="/stock-in/new" className="btn btn-primary"><Plus size={16} /> สร้างเอกสารรับเข้า</Link>
      </div>
      ...
      <div style={{ padding: '24px' }}>
        <div className="table-container">
          {loading ? (
            ...
            <div className="empty-state"><ArrowDownToLine size={40} color="#cbd5e1" /><p>ยังไม่มีเอกสารรับเข้า</p></div>
          ) : (
            <table>
              <thead>
                <tr>
              ...
              <tbody>
                {data.transactions.map((t: any) => (
                  <tr key={t.id}>
                    ...
                    <td>{t.supplier?.name || <span style={{ color: '#94a3b8' }}>-</span>}</td>
